...
struct cancellation_registry {
  ...
  static constexpr std::size_t max_bin_size = 1024;
...
private:
  ...
  bool try_register_from_idx(cancellation_registration* x, std::size_t idx);

  // ==== operations on bins (should the underlying container type and its bin operations be CPOs of some sort) ====

  void bin_insert_at_idx(cancellation_registration* x, std::size_t idx) { bin_insert(x, bins_[idx]); }
